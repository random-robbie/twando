...
<?php
   ...
   case 'tab5':
?>
<h2>Schedule a Tweet With Image</h2>
<?=$response_msg?>
<form method="post" action="" name="scheduletweet_image_form" id="scheduletweet_image_form"  onsubmit="ajax_tweet_settings_update('tab5','scheduletweet_image_form'); return false;">
Tweet content:<br />
<textarea name="tweet_content" id="tweet_content" class="input_box_style" style="height: 70px; width: 400px;" onkeyup="$('#count_box').val($('#tweet_content').val().length);"></textarea><br />
<br />Image Url
...
<br style="clear: both;" />
<input type="hidden" name="a" value="scheduletweet_image" />
</form>
<input type="submit" value="Schedule Tweet" class="submit_button_style" onclick="ajax_tweet_settings_update('tab5','scheduletweet_image_form');" />
<br /><br />
*Tweets are posted based on your servers PHP time, which might not be your local time. The current PHP date and time is <?=date(TIMESTAMP_FORMAT)?>.
<br />
<?php
